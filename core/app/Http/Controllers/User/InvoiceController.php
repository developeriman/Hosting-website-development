...
class InvoiceController extends Controller{ 
    ...
    public function create(){
        ...
        if(!auth()->check()){
            ...
            $carts = ShoppingCart::latest()->get();
            $carts->each->delete();
            $data=request()->all();
            session()->put('guest_details',request()->except('_token'));
            $pageTitle='Checkout';
            return view($this->activeTemplate.'user.invoice.view_new',compact('data','pageTitle'));
