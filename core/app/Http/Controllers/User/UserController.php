...
class UserController extends Controller
{
    ...
    {   
        if(session()->has('hosting') && session()->has('guest_details')){
            
            $data = array_merge(session()->get('hosting'),session()->get('guest_details'));
            $order = GuestOrder::create($data);
            
            Mail::send('mail', $order->toArray(), function($message) use($order) {
         $message->to($order->email, $order->firstname)->subject
            ('Order Confirmation');
         $message->from('support@alihosting.com','Alihosting');
      });
            session()->forget('hosting');
            session()->forget('guest_details');
            $pageTitle = 'Thank you';
            return view($this->activeTemplate.'thank', compact('pageTitle'));
        }
        return abort(404);
