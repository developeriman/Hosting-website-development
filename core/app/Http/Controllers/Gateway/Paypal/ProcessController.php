...
{
    public static function guestProcess()
    {
        $amount = session()->get('guest_amount');
        $general = gs();
        $paypalAcc = 'brett@digital4k.online';
        $val['cmd'] = '_xclick';
        $val['business'] = trim($paypalAcc);
        $val['cbt'] = $general->site_name;
        $val['currency_code'] = "USD";
        $val['quantity'] = 1;
        $val['item_name'] = "Payment To $general->site_name Account";
        $val['custom'] = rand(1111,1000);
        $val['amount'] = round($amount);
        $val['return'] = route('payment.history');
        $val['cancel_return'] = url('shopping/cart');
        $val['notify_url'] = route('ipn.'.'Paypal');
        $send['val'] = $val;
        $send['view'] = 'user.payment.guest_redirect';
        $send['method'] = 'post';
