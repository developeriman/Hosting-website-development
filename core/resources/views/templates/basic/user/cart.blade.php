                            ...
                            <div class="row">
                                <div class="col-md-11 d-flex justify-content-between flex-wrap align-items-center">
                                    @if ($cart->product_id && !$cart->domain_setup_id && !$cart->domain_id)
                                        <div>
                                            <h6 class="d-inline" id="product_name">{{ __($cart->product->name) }}</h6>
                                            <a href="{{ route('shopping.cart.config.service', $cart->id) }}">
                                                <i class="la la-pencil"></i> @lang('Edit')
                                            </a>
                                            <span class="d-block" id="product_category">{{ __($cart->product->serviceCategory->name) }}</span>
                    ...
                    @if (count($carts))
                        <div class="text-center mt-3">
                            <form action="{{ route('user.invoice.create') }}" method="post">
                                @csrf
                                <input type="hidden" name="product" required value="" id="product">
                                <input type="hidden" name="amount" required value="{{ $carts->sum('after_discount') }}">
                                <div class="form-group py-2">
                                    <input type="text" class="form-control form--control h-45" name="firstname" placeholder="@lang('Enter your name')" required>
                                </div>
                                <div class="form-group py-2">
                                    <input type="text" class="form-control form--control h-45" name="mobile" placeholder="@lang('Enter your phone')" required>
                                </div>
                                <div class="form-group py-2">
                                    <input type="email" class="form-control form--control h-45" name="email" placeholder="@lang('Enter your email')" required>
                                </div>
...
@push('script')
<script>
    $('#product').val($('#product_name').text()+' '+$('#product_category').text());
</script>
